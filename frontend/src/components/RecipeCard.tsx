...
export default function RecipeCard({recipe, ingredients}: {recipe: Recipe, ingredients: string[]}){
  const usesText = usesWhatIngredients(recipe, ingredients)

  return (
    <View className="relative flex bg-[rgb(42,44,50)] m-2 mx-8 border border-[rgb(57,59,67)] rounded-3xl">
      <View className="absolute top-3 right-3 z-10"><FavoriteButton recipe={recipe} /></View>
      <Text className="text-white font-bold text-2xl pl-5 pt-4 pr-12">{recipe.title}</Text>
      <View className="flex flex-row">
        {recipe.similarity > 0 && <View className="ml-5 mt-2 p-1 px-2 bg-[rgb(237,84,19)] rounded-2xl"><Text className="font-bold">{Math.round((recipe.similarity)*100)}% Match</Text></View>}
